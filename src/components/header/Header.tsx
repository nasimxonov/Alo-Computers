    ...
    <header className="bg-[#1A1A1D] sticky top-0 left-0 z-50 shadow-xl">
      <div className="mc flex items-center justify-between py-5">
        ...
        <div className="header-right flex items-center gap-5    ">
          {user ? (
            <Avatar onClick={()=> toggleProfileDrawer()}  className="!bg-[#ff5b00] cursor-pointer font-[600] text-xl md:text-2xl !text-[white]" size="large">{user.firstName ?  user.firstName.slice(0,1) : "" }</Avatar>
          ) : (
            <button
