...
const CategorisCard = ({data}:{data:categoriesType}) => {
  return (
    <div  className="w-full relative h-[302px] md:h-[352px] rounded-[12px] overflow-hidden  ">
        <img className="w-full h-full  bg-[#000000]  opacity-80"  src={data.imageUrl} alt="BG" />
         <div className="absolute inset-0 bg-black/60  "></div>

        <div className='flex  flex-col gap-2 absolute bottom-4 left-4'>
                <h4 className='font-[500] text-2xl text-white sm:text-3xl '>{data.title} </h4>
                <h5 className='text-white'>from <span className='text-xl text-[#ff5b00] sm:text-2xl font-[700] '>{data.price.toLocaleString()} so'm</span></h5>
